...
async function getPostDetail(req, res) {
  ...
  try {
    const [rows] = await executeQuery(connection, postQuery, [req.params.boardID]);
    const row = rows[0];
    res.status(200).json({
      userId: row.user_id,
      ...
      isLiked: req.session.postLiked.indexOf(req.params.boardID) !== -1 ? 1 : 0,
    });
  } catch (err) {
...
async function updatePost(req, res) {
  const authorId = await getAuthorId("post", req.params.boardID);
  ...
  try {
    await executeQuery(connection, query, [title, content, time, req.params.boardID]);
...
async function deletePost(req, res) {
  ...
  try {
    await executeQuery(connection, query, [req.params.boardID]);
...
async function updateLike(req, res) {
  req.session.postLiked = req.session.postLiked || [];
  const connection = await getConnection();
  const index = req.session.postLiked.indexOf(req.params.boardID);
  ...
  try {
    ...
    // 좋아요를 누르지 않은 게시글일 경우
      ...
      req.session.postLiked.push(req.params.boardID);
      const query = `
      UPDATE post
...
async function createComment(req, res) {
  ...
  try {
    await executeQuery(connection, query, [boardID, userId, username, content, time]);
...
async function updateComment(req, res) {
  const authorId = await getAuthorId("comment", req.params.replyID);

  // replyID에 해당하는 댓글이 존재하지 않는 경우
  if (!authorId) {
  ...
  try {
    await executeQuery(connection, query, [content, time, req.params.replyID, req.params.boardID]);
    res.status(200).json({ message: "댓글 수정 성공" });
  } catch (err) {
    console.error("댓글 수정 실패", err);
    res.status(500).json({ message: "서버 에러: 댓글 수정에 실패했습니다." });
  } finally {
    endConnection(connection);
  }
}
...
async function deleteComment(req, res) {
  ...
  try {
    await executeQuery(connection, query, [req.params.replyID, req.params.boardID]);
...
// 댓글 추천 함수
async function updateCommentLike(req, res) {
  req.session.commentLiked = req.session.commentLiked || [];
  const connection = await getConnection();
  const index = req.session.commentLiked.indexOf(req.params.replyID);
  ...
  try {
    ...
    if (index !== -1) {
      ...
      await executeQuery(connection, query, [req.params.replyID]);
      res.status(200).json({ message: "댓글 추천을 취소했습니다." });
    }
    // 좋아요를 누르지 않은 댓글일 경우
    else {
      req.session.commentLiked.push(req.params.replyID);
      const query = `
      UPDATE comment
      SET like_count = like_count + 1
      WHERE id = ?`;
