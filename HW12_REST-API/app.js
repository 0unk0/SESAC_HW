...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        } else if (req.method === "POST") { // <--- 숙제 4, 5 해결하면 POST, PUT, DELETE 코드 리팩토링 필요!!!
            if(req.url === "/user") {
                let body = "";
        ...
        } else if (req.method === "PUT") { // <--- 숙제 4, 5 해결하면 POST, PUT, DELETE 코드 리팩토링 필요!!!
            // 요청을 수정할 때
        ...
        } else if (req.method === "DELETE") { // <--- 숙제 4, 5 해결하면 POST, PUT, DELETE 코드 리팩토링 필요!!!
            // 요청을 삭제할 때
            if (req.url.startsWith("/user")){
                const key = req.url.split('/')[2];
                delete users[key];
